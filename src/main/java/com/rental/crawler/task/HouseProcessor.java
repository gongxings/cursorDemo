package com.rental.crawler.task;
...
//    private KuaiDaiLiIP kuaiDaiLiIP;
    ...
    private void saveHouseInfo(Page page) {
        ...
        if (statusCode == 200 && !s.contains("请输入验证码") && !s.contains("请完成登录")) {
            ...
            //获取数据封装到对象中
            houseInfo.setTitle(html.css("div.house-title h1", "text").toString());
...
//                    houseInfo.setFloorHeight("");
            ...
            houseInfo.setStatus(Jsoup.parse(html.css("div.house-desc-item ul.f14 li span a.ah").nodes().get(0).toString()).text());
